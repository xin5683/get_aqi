var localStorage = {
setItem:function(name,value){this[name] = value},
getItem:function(name){return this[name]},
clear(){for(let i in this){delete this[i]}},
...
const  askQsutngaSX = "a88fI3SwJty76iAK";//AESkey，可自定义
const  asidcaQpmcFY = "bLO1GVnaDcpEKu7x";//密钥偏移量IV，可自定义

const  ackRRRdVTM8B = "dlNJyiOfpTK1fSYL";//AESkey，可自定义
const  aciM3xzQy89s = "fNk1FvtuDIS9couQ";//密钥偏移量IV，可自定义

const  dskZvlq6OAM8 = "hSeyZQP94nqy0H1u";//DESkey，可自定义
const  dsi6v5eQklAY = "xucTWQMlE79utMBT";//密钥偏移量IV，可自定义

const  dckYtiuGdzEA = "oGFRqZG24Z2pjYTo";//DESkey，可自定义
const  dcilmbQVTpp2 = "pCVafXKh8Q98i50a";//密钥偏移量IV，可自定义
...
function dGrW07cu2XB(pZyvidD) {
  pZyvidD = AES.decrypt(pZyvidD, askQsutngaSX, asidcaQpmcFY);
  return pZyvidD;
}

function dX57IvdE59(pZyvidD) {
  pZyvidD = DES.decrypt(pZyvidD, dskZvlq6OAM8, dsi6v5eQklAY);
  return pZyvidD;
}

function gL9Svlaj5wJaT2rD(key, period) {
...
function dNkrPT6kzFFvwvoC3qqv2L4H(data) {
    data = BASE64.decrypt(data);
    data = DES.decrypt(data, dskZvlq6OAM8, dsi6v5eQklAY);
    data = AES.decrypt(data, askQsutngaSX, asidcaQpmcFY);
    data = BASE64.decrypt(data);
    return data;
}
var pK8oFGPrCMW = (function(){
...
return function(mGKJZ6V7J, o4rkst){
    var aYBX = 'c87228b446a7c273f5d30777e3cda7a9';
    var cMQ9T = 'WEB';
    var tTgz6ix = new Date().getTime();
    ...
    var pZyvidD = {
      ...
      clienttype: cMQ9T,
      object: o4rkst,
      secret: hex_md5(aYBX + mGKJZ6V7J + tTgz6ix + cMQ9T + JSON.stringify(osuVcGu5tb(o4rkst)))
    };
    pZyvidD = BASE64.encrypt(JSON.stringify(pZyvidD));
    pZyvidD = AES.encrypt(pZyvidD, ackRRRdVTM8B, aciM3xzQy89s);
    return pZyvidD;
};
})();

function spTrDtdeUyAJlCm(mGKJZ6V7J, o7BYxTjpKh, cwkEp7IQm, pHo97oP) {
    const kUGY = hex_md5(mGKJZ6V7J + JSON.stringify(o7BYxTjpKh));

    const dwEDR = gL9Svlaj5wJaT2rD(kUGY, pHo97oP);
    if (!dwEDR) {
        var pZyvidD = pK8oFGPrCMW(mGKJZ6V7J, o7BYxTjpKh);
        $.ajax({
            url: 'api/historyapi.php',
            data: { hxe4YdxKu: pZyvidD },
            type: "post",
            success: function (dwEDR) {
                dwEDR = dNkrPT6kzFFvwvoC3qqv2L4H(dwEDR);
                o4rkst = JSON.parse(dwEDR);
                if (o4rkst.success) {
                    if (pHo97oP > 0) {
                      o4rkst.result.time = new Date().getTime();
                      localStorageUtil.save(kUGY, o4rkst.result);
                    }
                    cwkEp7IQm(o4rkst.result);
                } else {
                    console.log(o4rkst.errcode, o4rkst.errmsg);
                }
            }
        });
    } else {
